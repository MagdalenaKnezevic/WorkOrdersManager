...
<div class="container">
    <form action="fullWorkOrderWorker.php?id=<?php echo $workOrderId; ?>" method="post">
        ...
        <h2>Fills in the Executor :</h2>
        <label for="status">Status:</label>
        <select id="status" name="status" required>
            <option value="On hold" <?php echo $workOrder['status'] == 'On hold' ? 'selected' : ''; ?>>Na čekanju</option>
            <option value="Accepted" <?php echo $workOrder['status'] == 'Accepted' ? 'selected' : ''; ?>>Prihvaćeno</option>
            <option value="In process" <?php echo $workOrder['status'] == 'In process' ? 'selected' : ''; ?>>U tijeku</option>
            <option value="Finished" <?php echo $workOrder['status'] == 'Finished' ? 'selected' : ''; ?>>Završeno</option>
        </select><br>

        <div id="executorFields" style="display: none;">
            <label for="descriptionDone">Description of done work:</label>
            <textarea id="descriptionDone" name="descriptionDone"><?php echo htmlspecialchars($workOrder['descriptionDone']); ?></textarea><br>

            <label for="material">Materials used:</label>
            <textarea id="material" name="material"><?php echo htmlspecialchars($workOrder['material']); ?></textarea><br>

            <label for="endDate">End of work:</label>
            <input type="text" id="endDate" name="endDate" value="<?php echo htmlspecialchars($workOrder['endDate']); ?>"><br>

            <label for="workersNote">Note:</label>
            <textarea id="workersNote" name="workersNote"><?php echo htmlspecialchars($workOrder['workersNote']); ?></textarea><br>
        </div>
